...
| A.8.12 | Data leakage prevention | The local profile is structural DLP: no outbound data path exists. Every provider block in `.env.example` is annotated `# data leaves your infra: yes/no`. On the delegated profiles, `REDACTION_PROVIDER=patterns` masks direct identifiers (emails, phone numbers, IBAN, Swiss AVS numbers) at the LLM boundary: a decorator over the client redacts everything outbound, the user's question and history included, through one unbypassable choke point. Offline, deterministic, auditable. Named-entity redaction is Roadmap, gated on a measured redaction bench. | Implemented (Phase 1 structural + Phase 2 patterns); NER Roadmap |
