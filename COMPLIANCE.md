...
| A.5.18 | Access rights | Admin rights are granted explicitly via `AUTH_ADMIN_USERS`; document deletion is restricted to the uploader or an admin. With `OIDC_ISSUER` set, the API accepts JWT bearers from the operator's own IdP (signature via cached JWKS with rotation handling, issuer and audience enforced, roles from a configurable claim path covering Keycloak and Entra shapes); API keys stay available next to it. The SPA signs in against the IdP with Authorization Code + PKCE (public client, no secret in the browser) and refreshes tokens transparently; sign-out clears the local session. | Implemented (Phase 1 + Phase 2 OIDC, backend and SPA); Entra ID guidance Roadmap |
| A.5.28 | Collection of evidence | Every assistant message persists an immutable snapshot of the sources shown to the user (filename, section/page, excerpt, score, per-leg ranks) that **survives document deletion**, plus a `request_id` correlation key. Persistence runs in a `finally` block shielded from cancellation, so the audit row lands on every exit path: success, provider failure, even a client disconnect mid-answer (`error_code` records the cause). A dedicated append-only `audit_log` table records user actions (uploads, deletions, sharing changes) with a database-enforced immutability trigger, readable by admins over the metrics window (`GET /api/admin/audit`). | Implemented (Phase 1 + Phase 2 audit_log) |
...
| A.8.12 | Data leakage prevention | The local profile is structural DLP: no outbound data path exists. Every provider block in `.env.example` is annotated `# data leaves your infra: yes/no`. On the delegated profiles, `REDACTION_PROVIDER=patterns` masks direct identifiers (emails, phone numbers, IBAN, Swiss AVS numbers) at the LLM boundary: a decorator over the client redacts everything outbound, the user's question and history included, through one unbypassable choke point. Offline, deterministic, auditable. `REDACTION_PROVIDER=ner` adds PERSON-only NER on top, in the shape the redaction bench selected: names 99 %, direct identifiers 100 %, 2 % false positives on clean legal text (full-NER redaction measured at 47 % and rejected; bench/results/2026-08-redaction.md). | Implemented (Phase 1 structural + Phase 2 patterns and measured NER) |
...
| A.8.16 | Monitoring activities | `/healthz` (liveness, no dependencies) and `/readyz` (database ping + LLM and embeddings healthchecks, cached 10 s); Docker HEALTHCHECKs on every service. Metrics dashboard served from the typed per-message columns: usage, token totals, p50/p95 stage latencies and the top-cited documents over a 30-day window, admin-only. | Implemented (Phase 1 + Phase 2 dashboard) |
