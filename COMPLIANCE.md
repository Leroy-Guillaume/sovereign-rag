...
document passages retrieved for each question transit to the endpoint **you** configured — you
choose the operator, the contract, and the jurisdiction. Embeddings stay local by default
...
|---|---|---|---|
| A.5.15 | Access control | Every API route requires an API key mapped to a stable user id (`AUTH_API_KEYS`); only the liveness/readiness probes are anonymous. In `prod` the application refuses to boot with an empty key set. Per-document ACL (`document_permissions` + a SQL predicate applied inside each retrieval leg) is designed and scheduled. | Implemented (Phase 1); ACL Roadmap (Phase 2) |
...
|---|---|---|
| Art. 6 | Lawfulness, good faith, proportionality | Retrieval is scoped strictly to the corpus the operator chose to ingest; the system prompt instructs the model to answer **only** from retrieved context and to state explicitly when no relevant passage was found. Data is processed for the answering purpose only — there is no secondary use. |
...
- **No shell utilities added for probes.** The API image's Docker HEALTHCHECK runs
  ...
  already present in its Alpine base image — nothing is added to either image for probing.
...
|---|---|---|
| No per-document ACL | Any authenticated user can search, list, and read passages from all ingested documents. | Roadmap (Phase 2): `document_permissions` table + SQL predicate in each retrieval leg |
